...
internal class OnlineDataSource(private val service: MovieService) {
    ...
    suspend fun getOneMovie(movId: String): Result<OneMovieResponse> {
        return try {
            val response = service.getOneMovie(movId)
            if (response.isSuccessful) {
                Result.Succes(response.body()!!)
            } else {
                Result.Error(
    ...
    suspend fun getMovieLatest(): Result<List<MovieLatestResponse.MovieLatest>> {
        return try {
            val response = service.getMovieLatest()
            if (response.isSuccessful) {
                Result.Succes(response.body()!!.results)
            } else {
                Result.Error(
